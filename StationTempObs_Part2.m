...
%% 1a. Add all needed folders to your MATLAB path
% --> use "addpath" to add the "model_data" folder from the Part 2
% repository (this repository)
p = genpath("model_data");
addpath(p)
% --> use "addpath" to add the "ObsData" folder from the Part 1
% repository
p1 = genpath("ObsData");
addpath(p1)

%% 1b. Load in a list of all 18 stations and their corresponding latitudes and longitudes
...
P_recent = NaN(length(sta), 2); %<-- do the same thing just for values from RecentYear to today
...
for i = 1:length(sta)
    station = sta(i);
    [P_all_sta, P_recent_sta] = StationTempObs_LinearTrend(station, RecentYear);
    P_all(i,1) = P_all_sta(1);
    P_all(i,2) = P_all_sta(2);
    P_recent(i,1) = P_recent_sta(1);
    P_recent(i,2) = P_recent_sta(2);
end
...
% below
P_projected = NaN(length(sta), 2);
Projected_temp_average = NaN(length(sta), 1);
Projected_temp_std = NaN(length(sta), 1);%<--
...
for i = 1:length(sta)
    ...
    P_projected(i,2) = P_projected_sta(2);
end%<--
